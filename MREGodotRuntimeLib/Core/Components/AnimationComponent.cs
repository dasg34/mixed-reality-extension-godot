...
namespace MixedRealityExtension.Core.Components
{
	internal class AnimationComponent : ActorComponentBase
	{
		...
		private void Update()
		{
			...
			var animation = GetGodotAnimationComponent();
			if (animation == null) return;
			
			var trackCount = animation.GetTrackCount();
			for (int i = 0; i < trackCount; i++)
			{
				var trackPath = animation.TrackGetPath(i);
				var trackIsEnabled = animation.TrackIsEnabled(i);
				if (!GetAnimationData(trackPath, out AnimationData animationData)
					|| animationData.Managed
					|| animationData.Enabled == trackIsEnabled
				)
					continue;
				...
				{
					_animationData.Remove(trackPath);
			...
			bool enabled)
		{
			...
			bool SlerpQuaternion(out Quat dest, Quat start, Quat end, float t)
			{
				if (end != null)
				{
					dest = Quaternion.SlerpUnclamped(start, new Quat(end.x, end.y, end.z, end.w), t);
					return true;
				}
				dest = Quat.Identity;
				return false;
