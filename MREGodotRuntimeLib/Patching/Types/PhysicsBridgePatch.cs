		...
		private List<Snapshot.TransformInfo> ConvertTransformBLOB(byte[] blob)
		{
			...
			using (BinaryReader binaryReader = new BinaryReader(memoryStream))
			{
				for (int i = 0; i < memoryStream.Length / 48; i++)
				{
					...
					//GD.Print($"transformInfo.Transform r: ({transformInfo.Transform.Position.x}, {transformInfo.Transform.Position.y}, {transformInfo.Transform.Position.z}");
		...
		private byte[] ConvertTransformList(List<Snapshot.TransformInfo> list)
		{
			...
			using (BinaryWriter binaryWriter = new BinaryWriter(memoryStream))
			{
				foreach (var transformInfo in list)
				{
					...
					//GD.Print($"transformInfo.Transform  : ({transformInfo.Transform.Position.x}, {transformInfo.Transform.Position.y}, {-transformInfo.Transform.Position.z}");
				}
